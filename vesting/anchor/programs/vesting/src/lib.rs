...
declare_id!("MQqKKGBhVXFH8Jsq4kPiD3zQyDm3PtQNRKkN3toczpK");
...
pub struct CreateVestingAccount<'info> {
    ...
    #[account(
        init,
        token::mint = mint,
        token::authority = treasury_token_account, // This is the PDA so the authority is the treasury token account so it can be used to transfer tokens
        payer = signer,
...
pub struct EmployeeAccount {
    ...
    pub start_time: i64,
    pub end_time: i64,
    pub cliff_time: i64, //wait time before tokens are unlocked for claim
